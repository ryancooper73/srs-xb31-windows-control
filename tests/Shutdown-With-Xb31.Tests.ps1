...
$tempLeaf = "SRS-XB31-Shutdown-DryRun-$([Guid]::NewGuid().ToString('N'))"
...
'@ | Set-Content -LiteralPath $fixtureHelperPath -Encoding ASCII
    ...
    $output = @($observedOutput | ForEach-Object { $_.Text })
    $exitCode = $LASTEXITCODE

    if ($exitCode -ne 0) {
        throw "FAIL: dry run exited with $exitCode. Output: $($output -join ' | ')"
    }

    if ($output -notcontains 'Shutdown: Windows shutdown skipped (dry run)') {
        throw "FAIL: dry run did not explicitly skip Windows shutdown. Output: $($output -join ' | ')"
    }

    $helperExit = $observedOutput | Where-Object { $_.Text -eq 'Shutdown: XB31 helper exit 0' } | Select-Object -First 1
    $shutdownSkipped = $observedOutput | Where-Object { $_.Text -eq 'Shutdown: Windows shutdown skipped (dry run)' } | Select-Object -First 1
    if ($null -eq $helperExit) {
        throw "FAIL: dry run did not report a successful helper exit. Output: $($output -join ' | ')"
    }

    $gracePeriod = $shutdownSkipped.ObservedAtUtc - $helperExit.ObservedAtUtc
    if ($gracePeriod.TotalMilliseconds -lt 1800) {
        throw "FAIL: Windows shutdown path was not held for the two-second XB31 grace period. Observed $([Math]::Round($gracePeriod.TotalMilliseconds)) ms."
    }

    Write-Output 'PASS: combined shutdown dry run honored the XB31 grace period and skipped Windows shutdown'
}
finally {
